  ...
  if (options.message == ""){
      ...
      options.contexts.forEach((context) => {
        if (context.name === "invites"){
          options.message = textResponses.inviteUserResponses.initialMessage
          var isEqual
          ...
          if (isEqual){
            options.message = user.displayName + " is already invited"
          }
          else{
            options.message += user.displayName +", email: "+ user.mail
      ...
      if (options.speech==""){
        options.message = " Couldn't invite " + user.displayName
        reject(textResponses.inviteUserResponses.errorMessage)
      }
    }
    resolve("Success")
  })
...
function showInvites(options, callback){
  ...
  if (!invitesContext){
    options.message = textResponses.showInvitesResponses.noInvitesMessage
    callback(options)
  }
  var invites = invitesContext.parameters.invites
  options.message = textResponses.showInvitesResponses.initialMessage
...
function deleteInvite(options, callback){
  ...
  if (!commons.getContext(options.contexts, "invites")){
    ...
    options.message = "Couldn't find " + ((userData.name) ? userData.name: userData.email)
    callback(options)
  }
  var invitesContext = commons.getContext(options.contexts, "invites")
  var invites = invitesContext.parameters.invites
  for (var i in invites){
    const uninvitedMessage = " was uninvited "
    if (userData.name && userData.lastname && invites[i].emailAddress.name.toLowerCase().includes(userData.name.toLowerCase())
      ...
      options.message = invites[i].emailAddress.name + uninvitedMessage
      invites.splice(i, 1)
      callback(options)
  ...
  options.message = userData.email + "Couldnt find " + ((userData.name) ? userData.name: userData.email)
  callback(options)
}
